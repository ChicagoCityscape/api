...
| chicago_owned  | Optional. Set this to `true` to return information about the potential City-owned land status of each parcel in the response. Look for the `chicago_owned` property, which is an object with two properties (current status, and the date on which that status was applied).
