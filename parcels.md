...
| chicago_owned  | Optional. Set this to `true` to return information about the potential City-owned land status of each parcel in the response. Look for the `chicago_owned` property, which is an object with two properties (current status, and the date on which that status was applied). Tip: Don't set a `prop_class` when using `chicago_owned` since many Chicago-owned properties are unclassifed.
| prop_class     | Optional. Filter: Returns only parcels in a given comma-separated list of Cook County property classes. [Review all the possible property classes](https://www.chicagocityscape.com/guides/propertyclasses.php), used by the Cook County Assessor.
